...
<script>
    $(document).ready(function()
    {
        ...
        $.fn.myFunctionDelete = function(ID){
            $.confirm({
                title: 'Confirm!',
                content: 'Voulez vous vraiment supprimer cette promotion!!',
                confirm: function(){
                    $.post("<?php echo site_url('/promotion/delete'); ?>",     
                        {
                            ID: ID
                        });
                    window.location.href = "<?php echo site_url('promotion'); ?>";
                },
                cancel: function(){
                    $.alert('Canceled!')
                }
...
<script>
    $(document).ready(function()
    {
        $("#submit").click(function()
        {
            ...
            var pourc=$("#pourcentage").val();
            var id_repas=$("#id_repas").val();
            if ((date_fin == "") || (date_deb == "")||(pourc == "") || (id_repas == null))  {
                ...
                $.post("<?php echo site_url('/promotion/create'); ?>",   //"./client/create"
                    {
                        date_fin: date_fin, date_deb: date_deb, pourc: pourc, id_repas: id_repas
                    }
                );
                window.location.href = "<?php echo site_url('promotion'); ?>";
                ...
                <div class="box col-md-12">
                    <div class="box-inner">
                        <div class="box-header well" data-original-title="">
                            <h2><i class="glyphicon glyphicon-user"></i> Datatable Promotion</h2>
                        ...
                        <div class="box-content">
                            <table class="table table-striped table-bordered bootstrap-datatable datatable responsive">
                                ...
                                <tbody>
                                <?php  if (isset($promo)) :foreach ($promo as $row) : ?>
                                    <tr>
                                        <td class="center"><?php echo $row->ID_PRO; ?></td>
                                        <td class="center"><?php echo $row->NOM; ?></td>
                                        <td class="center"><?php echo $row->POURCENTAGE; ?></td>
                                        <td class="center"><?php echo $row->DATE_DEBUT; ?></td>
                                        <td class="center"><?php echo $row->DATE_FIN; ?></td>
                                        <td >
                                            <?php echo anchor("/promotion/edit_page/$row->ID_PRO",'Edit','class="btn btn-info "'); ?> <input type="button" class="btn btn-danger " value="Delete" onclick="$.fn.myFunctionDelete(<?php echo $row->ID_PRO ?>)"; />
                                        </td>
                                    </tr>
                                <?php endforeach ?>
                                </tbody>
                    ...
                    <div class="modal-content">
                        <div class="row">
                            <div class="box col-md-12">
                                <div class="box-inner">
                                    <div class="box-header well" data-original-title="">
                                        <h2><i class="glyphicon glyphicon-edit"></i> Add pomotion</h2>
                                    </div>
                                    <div class="box-content">
                                        <div id="alert">
                                        </div>
                                            ...
                                            <form role='form'>
                                                <label>Repas<span>(Required Field)</span></label>
                                                <select class="form-control" id="id_repas" >
                                                    <?php
                                                    foreach($repas as $row)
                                                    { ?>
                                                        <option value="<?php echo $row->ID_REPAS; ?>"  > <?php echo $row->NOM; ?> </option>;
                                                    <?php }
                                                    ?>
                                                </select>
                                            <div class='form-group'>
                                            <label>Pourcentage <span>(Required Field)</span></label>
                                            <input type='text' class='form-control' id='pourcentage'/>
                                            </div>
                                                <div class='form-group'>
                                                    <label>Date début <span>(Required Field)</span></label>
                                                    <input type='date' class='form-control' id='date_deb'/>
                                                </div>
                                                <div class='form-group'>
                                                    <label>Date fin <span>(Required Field)</span></label>
