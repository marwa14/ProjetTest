...
<script>
    $(document).ready(function()
    {
        $.fn.myFunctionDelete = function(ID){
            ...
            $.post("<?php echo site_url('/restaurant/delete'); ?>",     //"./client/create"
                {
                    ID: ID
                });
...
<script>
    $(document).ready(function()
    {
        $("#submit").click(function()
        {
            ...
            var nb_table=$("#nb_table").val();
            var nb_pers_table=$("#nb_pers_table").val();
            var featured = 0;
            if ((nom=="") || (description=="") || (region=="") || (localisation=="") ||(horaire=="") || (moyen=="") || (service=="") || (nb_table=="") || (nb_pers_table=="") ) {
                ...
                if (isNaN(nb_table))
                {
                    $( "#alert" ).addClass( "alert alert-danger" );
                    $("#alert")
                        .text("nb_table doit être un entier")
                }
               else  if (isNaN(nb_pers_table))
                {
                    $( "#alert" ).addClass( "alert alert-danger" );
                    $("#alert")
                        .text("nb_pers_table doit être un entier")
                }
                else {
                    ...
                    $.post("<?php echo site_url('/restaurant/create'); ?>",   //"./client/create"
                        {
                            nom: nom,
    ...
    $(document).ready(function()
    {
        $("#ok").click(function()
        {
            ...
            if ((nom=="") || (description=="") || (region=="") || (localisation=="") ||(horaire=="") || (moyen=="") || (service=="") || (nb_table=="") || (nb_pers_table=="") ) {
                ...
                $.post("<?php echo site_url('/restaurant/edit'); ?>",   //"./client/create"
                    {
                        nom: nom, description: description, region: region, localisation: localisation, horaire: horaire, moyen: moyen, service: service, nb_table: nb_table, nb_pers_table:nb_pers_table, featured:featured, id:id
                    }
                );
            ...
            <div class="sidebar-nav">
                <div class="nav-canvas">
                    ...
                    <ul class="nav nav-pills nav-stacked main-menu">
                        ...
                        <li><a class="ajax-link" href="http://localhost/CodeIgniter-3.0.6/index.php/restaurant"><i class="glyphicon glyphicon-home"></i><span> Profil </span></a>
                        </li>
            ...
            <div class="row">
                ...
                <div class="box col-md-12">
                    <div class="box-inner">
                        <div class="box-header well" data-original-title="">
                            <h2><i class="glyphicon glyphicon-user"></i> Coordonnées restaurant</h2>
                            ...
                            <?php if (isset($restaurant)): foreach ($restaurant as $row) : ?>
                        ...
                        <div id="alert">
                            <form role="form">
                                <div class="form-group">
                                    <label>Id </label>
                                    <br>
                                    <label id="id" ><?php echo $row->ID; ?> </label>
                                </div>
                                <div class="form-group">
                                    <label>Nom <span>(Required Field)</span></label>
                                    <input type="text" class="form-control" id="nom" value="<?php echo $row->NOM; ?>" />
                                </div>
                                <div class="form-group">
                                    <label>Description <span>(Required Field)</span></label>
                                    <textarea  class="form-control autogrow"  id="description"  ><?php echo $row->DESCRIPTION; ?> </textarea>
                                </div>
                                <div class="form-group" >
                                    <label>Region <span>(Required Field)</span></label>
                                    <input type="text" class="form-control"  id="region" value="<?php echo $row->REGION; ?>" />
                                </div>
                                <div class="form-group" >
                                    <label>Localisation<span>(Required Field)</span></label>
                                    <input type="text" class="form-control"  id="localisation" value="<?php echo $row->LOCALISATION; ?>" />
                                </div>
                                <div class="form-group" >
                                    <label>Horaire d'ouverture <span>(Required Field)</span></label>
                                    <input type="text" class="form-control"  id="horaire" value="<?php echo $row->HORAIRE_OUVERTURE; ?> "/>
                                </div>
                                <div class="form-group" >
                                    <label>moyen de paiement <span>(Required Field)</span></label>
                                    <input type="text" class="form-control"  id="moyen" value="<?php echo $row->MOYEN_PAIEMENT; ?>"/>
                                </div>
                                <div class="form-group" >
                                    <label>service <span>(Required Field)</span></label>
                                    <input type="text" class="form-control"  id="service" value="<?php echo $row->SERVICE; ?>"/>
                                </div>
                                <div class="form-group" >
                                    <label>nombre de table <span>(Required Field)</span></label>
                                    <input type="text" class="form-control"  id="nb_table" value="<?php echo $row->NB_TABLE; ?>"/>
                                </div>
                                <div class="form-group" >
                                    <label>nombre de personne par table <span>(Required Field)</span></label>
                                    <input type="text" class="form-control"  id="nb_pers_table" value="<?php echo $row->NB_PERS_TABLE; ?>" />
                                </div>
                                <?php if ($row->FEATURED==1)
                                    echo "<input type='checkbox'  id='Featured' checked='true'  class='checkbox-inline'/>"; else echo "<input type='checkbox' class='checkbox-inline' id='Featured'/>";?> Featured
                                <br>
                                <br>

                                <input type="button"  class="btn btn-primary" value="Save" id="ok" />
                            ...
                            <?php else:  ?>
                                <form role="form">
                                    ...
                                    <div class="form-group" >
                                        <label>moyen de paiement <span>(Required Field)</span></label>
                                        <input type="text" class="form-control"  id="moyen" />
                                    </div>
                                    <div class="form-group" >
                                        <label>service <span>(Required Field)</span></label>
                                    ...
                                    <div class="form-group" >
                                        <label>nombre de table <span>(Required Field)</span></label>
                                        <input type="text" class="form-control"  id="nb_table" />
                                    </div>
                                    <div class="form-group" >
                                        <label>nombre de personne par table <span>(Required Field)</span></label>
                                        <input type="text" class="form-control"  id="nb_pers_table"  />
                                    </div>

                                    <input type='checkbox' class='checkbox-inline' id='Featured'/> Featured
                                    <br>
                                    <br>
                ...
                <!--/span-->
                    ...
                    <iframe
                        ...
                        src="https://www.google.com/maps/embed/v1/place?key=AIzaSyCHFFXm2sAgQIyubdDPYaTe2G94E4olv2c
                   &q=<?php echo  $loc; ?>" allowfullscreen>
                    </iframe>
            </div><!--/row-->
