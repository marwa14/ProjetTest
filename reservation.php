...
<script>
    $(document).ready(function()
    {
        ...
        $.fn.myFunctionDeny = function(ID){
            $.confirm({
                title: 'Confirm!',
                content: 'Voulez vous vraiment annuler cette réservation!!',
                confirm: function(){
                    $.post("<?php echo site_url('/reservation/annuler'); ?>",     //"./client/create"
                        {
                            ID: ID
                        });
...
<script>
    $(document).ready(function()
    {
        ...
        $.fn.myFunctionAccept = function(ID){
            $.confirm({
                title: 'Confirm!',
                content: 'Voulez vous vraiment accepter cette réservation!!',
                confirm: function(){
                    $.post("<?php echo site_url('/reservation/accepter'); ?>",     //"./client/create"
                        {
                            ID: ID
                        });
                    window.location.href = "<?php echo site_url('reservation'); ?>";
...
<script>
    $(document).ready(function()
    {
        ...
        $.fn.myFunctionDelete = function(ID){
            ...
            $.confirm({
                title: 'Confirm!',
                content: 'Voulez vous vraiment supprimer cette reservation!!',
                confirm: function(){
                    $.post("<?php echo site_url('/reservation/delete'); ?>",     //"./client/create"
                        {
                            ID: ID
                        });
                    window.location.href = "<?php echo site_url('reservation'); ?>";
                ...
                <div class="box col-md-12">
                    <div class="box-inner">
                        <div class="box-header well" data-original-title="">
                            <h2><i class="glyphicon glyphicon-user"></i> Datatable Carte</h2>
                        ...
                        <div class="box-content">
                            <table class="table table-striped table-bordered bootstrap-datatable datatable responsive">
                                ...
                                <tbody>
                                <?php  if (isset($reser)) :foreach ($reser as $row) : ?>
                                    <tr>
                                        <td class="center"><?php echo $row->NOM; ?></td>
                                        <td class="center"><?php echo $row->PRENOM; ?></td>
                                        <td class="center"><?php echo $row->EMAIL; ?></td>
                                        <td class="center"><?php echo $row->NB_PLACE; ?></td>
                                        <td class="center"><?php echo $row->DATE_RES; ?></td>
                                        <td class="center"><?php echo $row->HORAIRE; ?></td>
                                        <td class="center"><?php echo $row->TELEPHONE; ?></td>
                                        <td class="center"><?php echo $row->TYPE_REPAS; ?></td>
                                        <td >
                                            <?php if ($row->ETAT ==""){?>
                                            <input type="button" class="btn btn-info" value="Accept" onclick="$.fn.myFunctionAccept(<?php echo $row->ID_RES ?>)";>
                                             <input type="button" class="btn btn-danger " value="Deny" onclick="$.fn.myFunctionDeny(<?php echo $row->ID_RES ?>)";>
                                            <?php }?>

                                            <input type="button" class="btn btn-danger"  value="Delete" onclick="$.fn.myFunctionDelete(<?php echo $row->ID_RES ?>)";/>
                                            <?php if ($row->ETAT =="deny") echo "<span class='label label-danger'>Deny</span>";  else  if ($row->ETAT =="accept")echo "<span class='label label-success'>Accept</span>";?>
                    ...
                    <div class="modal-content">
                        <div class="row">
                            <div class="box col-md-12">
                                <div class="box-inner">
                                    <div class="box-header well" data-original-title="">
                                        <h2><i class="glyphicon glyphicon-edit"></i> Add Carte</h2>
                                    </div>
                                    <div class="box-content">
                                        <div id="alert">
                                        </div>
                                        <?php if ($rest==null) echo "vous n'avez pas de restaurant  !!!!!" ;
                                        else {
                                            echo "<form role='form'>";
                                            echo " <div class='form-group'>";
                                            echo "<label>Nom <span>(Required Field)</span></label>";
                                            echo "<input type='text' class='form-control' id='nom'/>";
                                            echo "</div>";
                                            echo "<input type='button'  class='btn btn-primary' value='Save' id='submit' />";
